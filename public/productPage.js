...
const addProduct = async () => {
    try {
        const purchaseOrderDetails = ['ID', 'name', 'color', 'size', 'price', 'barcode', 'weight', 'category', 'quantityinstore', 'quantityinwarehouse', 'safetystocklevel'];
        const newPurchaseOrder = {};
        
        purchaseOrderDetails.forEach(detail => {
            let value = prompt(`Enter ${detail}:`);
            if (detail === 'quantityinstore' || detail === 'quantityinwarehouse' || detail === 'safetystocklevel') {
                // Parse quantity values as integers
